...
Shield::Shield() : m_name(""), m_description(""), m_defenseCoeff(0.0), m_defenseType("") {}

Shield::Shield(const QString &name, const QString &description,
               double defenseCoeff, const QString &defenseType)
    : m_name(name), m_description(description),
    m_defenseCoeff(defenseCoeff), m_defenseType(defenseType) {}
...
bool Shield::isValidName() const { return !m_name.trimmed().isEmpty(); }
bool Shield::isValidDescription() const { return !m_description.trimmed().isEmpty(); }
bool Shield::isValidDefenseCoeff() const { return m_defenseCoeff > 0.0; }
bool Shield::isValidDefenseType() const { return VALID_TYPES.contains(m_defenseType); }
