...
Car& Car::operator=(const Car& other) {
    if (this != &other) {  
        ...
        cout << "Выполнено присваивание для автомобиля "
            << brand << " " << model << " (" << stateNumber << ")" << endl;
    }
    return *this;  
}
...
Car Car::operator+(const Car& other) const {
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<int> brandDist(0, 1);
    ...
    cout << "Сгенерирован госномер: " << result.stateNumber << endl;
    ...
    result.carStereo = this->carStereo;
    result.currentWorkshop = this->currentWorkshop;
    result.fuel = this->fuel;
    result.fuelTankCapacity = this->fuelTankCapacity;
    result.fuelConsumption = this->fuelConsumption;
...
Car Car::operator-(const Car& other) const {
    ...
    for (const auto& item : this->things) {
        bool found = false;
...
Car Car::operator/(const Car& other) const {
    ...
    // Удаляем дубликаты из общих вещей
    if (!result.things.empty()) {
        sort(result.things.begin(), result.things.end());
        auto last = unique(result.things.begin(), result.things.end());
        result.things.erase(last, result.things.end());
    }
